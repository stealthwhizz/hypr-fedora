...
- **System crashing/rebooting every few minutes, eventually forcing a fallback to KDE Plasma** — the real root cause behind a long, confusing round of "keybinds don't work" debugging that turned out to be a red herring (the system was crashing out from under Hyprland, not misconfigured). Traced through `journalctl --list-boots`: the machine had rebooted 5 times in ~3.5 hours, with the two most recent Hyprland boots each lasting only 4-5 minutes. Kernel log in the boot right before the crash showed `nouveau` (the open-source NVIDIA driver) GSP-firmware RPC warnings (`r535_gsp_msgq_wait`, `r535_mmu_vaspace_del`) about a minute before each reboot — this is very likely the same underlying driver instability behind the earlier hybrid-GPU display bug documented above, just severe enough this time to crash the whole system. This machine has hybrid graphics (Intel Iris Xe on `card1`/`i915`, stable; NVIDIA RTX 3060 on `card2`/`nouveau`, the crashing one) with no GPU preference set anywhere, so Hyprland's backend (aquamarine) was auto-picking NVIDIA as the primary render GPU — exactly the unstable path — for no better reason than it being the more powerful card. Fixed by explicitly preferring the Intel GPU: `AQ_DRM_DEVICES=/dev/dri/card1:/dev/dri/card2` (Intel first = primary, NVIDIA still listed second since the external HDMI port is physically wired through it, confirmed earlier this session). Set in **two places** for reliability: `env.lua` (`hl.env(...)`, in case Hyprland's own Lua config load happens early enough to matter) and `~/.config/environment.d/10-gpu.conf` (systemd/`pam_systemd`-applied at login, before any compositor starts at all — the more robust of the two, since GPU backend init likely happens before Lua config parsing). **Unverified** — could not be tested live: Hyprland wasn't running at the time this was diagnosed (the user had already fallen back to KDE Plasma), and starting a fresh Hyprland session to test would have meant logging out of the working KDE session. Needs a real login to Hyprland to confirm the crash is actually gone; if it recurs, the next step would be trying NVIDIA's proprietary driver (`akmod-nvidia` via RPM Fusion) instead of `nouveau`, which is more mature for GSP-firmware GPUs (all NVIDIA Turing/Ampere+ cards, including this RTX 3060, require GSP firmware).

- **"hyprland-guiutils not installed" warning persisting even after installing it** — confirmed the exact check straight from Hyprland's own binary (`strings /usr/bin/Hyprland`): it's a plain PATH lookup for `hyprland-dialog` (`"hyprland-dialog missing from PATH, cannot start ANRManager"`), not a package-manager-level check. `/usr/bin/hyprland-dialog` genuinely exists (confirmed via `rpm -ql hyprland-guiutils`) and `/usr/bin` is on Hyprland's PATH, yet the warning still fires — a known, harmless version-lag mismatch between this `sdegler` COPR build and the `hyprland-guiutils` package, matching a real upstream issue ([hyprwm/hyprland-guiutils#11](https://github.com/hyprwm/hyprland-guiutils/issues/11)) closed as "not planned" (i.e. accepted as a benign packaging quirk, not something with a real fix). Since it's confirmed 100% cosmetic — never blocks startup, unrelated to the separate GPU crash issue below — silenced it via Hyprland's own official option, `misc.disable_hyprland_guiutils_check = true` in `settings.lua`, rather than continuing to chase an unfixable warning.
...
3. Copy `hypr/` → `~/.config/hypr/`, `matugen/` → `~/.config/matugen/`, `kitty/kitty.conf` → `~/.config/kitty/kitty.conf`, `bin/display-switch` → `~/.local/bin/display-switch` (`chmod +x` it), `environment.d/10-gpu.conf` → `~/.config/environment.d/10-gpu.conf` (**only if you have the same hybrid Intel+NVIDIA setup** — see the GPU crash entry below; skip/adjust the device paths otherwise).
4. `hyprctl reload`.
