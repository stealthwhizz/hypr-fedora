...
- **Calendar's schedule strip rebuilt to pull from Google Calendar** — the original feature (`scripts/quickshell/calendar/schedule/get_schedule.py`) scraped a Danish school timetable site via Selenium, tied to the original author's own Firefox profile (`/home/ilyamiro/.mozilla/firefox/schedule.special`) and invoked through `nix-shell` (`schedule_manager.sh`) — which isn't even installed on this system, so this was never going to work here at all, not just a wrong-profile issue. Rewrote `get_schedule.py` from scratch: pulls today's events from a Google Calendar's private iCal feed (no OAuth/API key needed — just the account's own unlisted `.ics` URL from Calendar Settings → Integrate calendar → "Secret address in iCal format") and renders them into the *exact same JSON shape* the QML UI already expects, so `CalendarPopup.qml` needed zero changes. Parses the ICS format directly with stdlib only (no new dependency — `icalendar` isn't installed, `requests` already is) rather than pulling in a library. All-day events are deliberately skipped: the UI is a fixed horizontal timeline (originally shaped for an 8:30–15:40 school day, kept as a 07:00–22:00 waking-hours window here) that a full-day event doesn't fit visually. `schedule_manager.sh` simplified to a plain `python3` call, dropping the dead `nix-shell`/`shell.nix` dependency entirely. Verified with a synthetic ICS feed (two timed events + one all-day, exercised locally without touching the network): parser correctly extracted both timed events, correctly skipped the all-day one, computed gaps and layout widths matching the original's math, and the calendar popup opened cleanly end-to-end with the new script wired in (confirmed via `hyprctl layers`, no 1×1 stuck-render or crash). **Needs one thing from you to show real data**: your own private iCal URL(s) in `scripts/quickshell/calendar/schedule/.env` as `GOOGLE_CALENDAR_ICS_URLS=url1,url2,...` (comma-separated, gitignored, never committed). Without it, the strip correctly shows "No Google Calendar configured" instead of silently failing. Supports multiple calendars in one account merged into a single timeline — one secret iCal URL per calendar (each calendar has its own, found the same way under Settings → that calendar → Integrate calendar), events sorted together and deduped if the exact same event appears on more than one of them. One calendar's URL failing to fetch doesn't take down the others.
