...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    ( matugen image "$FINAL_THUMB" --prefer saturation --type scheme-vibrant || true; bash "$RELOAD_SCRIPT" || true ) &
                `;
        ...
        const fullScript = `
            ...
            ${wallpaperCmd}
            ( matugen image "${escThumb}" --prefer saturation --type scheme-vibrant || true; bash "${escReload}" || true ) &
        `;
