...
return {
    ...
    -- Apps
    -- foot/thunar aren't installed on this machine; kitty/nautilus are,
    ...
    -- volume-mixer GUI) rather than guess at an unverified third-party repo.
    ...
    volumeStep                 = 10,
    volumeMax                  = 100,
    -- sweet-cursors isn't installed anywhere on this system (same class of
    -- bug serpantinum documented: theme mismatch -> flicker between GTK's
    -- fallback and Hyprland's own compositor-level cursor rendering).
    -- Bibata-Modern-Classic (serpantinum's fix) has no verified Fedora COPR
    -- either - using breeze_cursors instead, since it's already installed as
    -- this KDE Plasma spin's own system default (breeze-cursor-theme
    -- package), sidestepping the "not installed" root cause entirely rather
    -- than trading one availability risk for another.
