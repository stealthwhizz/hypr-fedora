...
#
# Why this is needed: PAM (pam_gnome_keyring.so, wired into /etc/pam.d/sddm -
# this machine's login manager is plasmalogin, an SDDM derivative, not GDM)
# does start and unlock a login keyring at actual login time. But Hyprland's
# own process never inherits those env vars (checked
...
# prompt to create/unlock its own keyring. This exact bug (and this exact fix)
# was already diagnosed and proven live against this same OS+session stack
# while porting the serpantinum shell - ported here verbatim since the
# underlying PAM setup is unchanged and the bug is near-certain to recur.
#
