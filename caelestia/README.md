...
- **Nothing in this folder has been live-tested yet** — `~/.config/hypr` and `~/.config/caelestia` didn't exist before this port, and this README was written *before* first login, not after (unlike serpantinum's, where every fix says "Verified live"). Treat every "Unverified" item above as a real open question until tested. See Verification below for the plan.
